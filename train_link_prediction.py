    ...
    for run in range(args.num_runs):
        ...
        if args.model_name == 'TGAT':
            ...
            dynamic_backbone=Liquid(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                           time_feat_dim=args.time_feat_dim, model_name=args.model_name, num_layers=args.num_layers, num_heads=args.num_heads,
                                           dropout=args.dropout, src_node_mean_time_shift=src_node_mean_time_shift, src_node_std_time_shift=src_node_std_time_shift,
                                           dst_node_mean_time_shift_dst=dst_node_mean_time_shift_dst, dst_node_std_time_shift=dst_node_std_time_shift, device=args.device,structure_dim=node_raw_features.shape[1], max_input_sequence_length=args.max_input_sequence_length)
        else:
        ...
        save_model_folder = f"./saved_models/{args.model_name}/{args.dataset_name+str(args.batch_size)}/{args.save_model_name}/"
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings,exloss1 = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                          dst_node_ids=batch_neg_dst_node_ids,
                                                                          node_interact_times=batch_node_interact_times,
                                                                          edge_ids=None,
                    ...
                    # get temporal embedding of source and destination nodes
                    # two Tensors, with shape (batch_size, node_feat_dim)
                    batch_src_node_embeddings, batch_dst_node_embeddings,exloss2 = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                          dst_node_ids=batch_dst_node_ids,
                                                                          node_interact_times=batch_node_interact_times,
                                                                          edge_ids=batch_edge_ids,
                ...
                lossy=loss_func(input=predicts, target=labels)
                if args.model_name in ['Liquid']:
                    loss = lossy + (exloss1+exloss2)
                else:
                    loss = lossy
                ...
                train_idx_data_loader_tqdm.set_description(f'Epoch: {epoch + 1}, train for the {batch_idx + 1}-th batch, train loss: {lossy.item()}')
            ...
            logger.info(f'Epoch: {epoch + 1}, learning rate: {optimizer.param_groups[0]["lr"]}, train loss: {np.mean(train_losses):.4f},train time: {end-start:.2f}s')
